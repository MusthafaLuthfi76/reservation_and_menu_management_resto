...
api.get(
  ...
  asyncH(async (req, res) => {
    ...
    const doc = await db
      .collection("orders")
      .findOne({ table_number, status: "ordered" }, { projection: { _id: 0 } });
...
api.post(
  ...
  asyncH(async (req, res) => {
    ...
    if (order.status !== "ordered") return res.status(400).json({ detail: "Cannot add items to a paid order" });
...
api.post(
  ...
  asyncH(async (req, res) => {
    ...
    const order = await db.collection("orders").findOne({ id: req.params.id }, { projection: { _id: 0 } });
    if (!order) return res.status(404).json({ detail: "Order not found" });
    if (order.status === "paid") return res.status(400).json({ detail: "Order already paid" });
    const update = { status: "paid", payment_method: method, finish_time: nowIso() };
...
async function seed() {
  ...
  if (!existing) {
    ...
    await db
      .collection("users")
      .updateOne({ email: ADMIN_EMAIL }, { $set: { password_hash: hashPassword(ADMIN_PASSWORD) } });
  }

  if ((await db.collection("menu_items").countDocuments()) === 0) {
    const samples = [
      { name: "Edamame", description: "Lightly salted young soybeans", price: 480, category: "appetizer",
        image_url: "https://images.unsplash.com/photo-1564834744159-ff0ea41ba4b9?w=600" },
      { name: "Salmon Sashimi", description: "Five slices of premium Norwegian salmon", price: 1380, category: "appetizer",
      ...
      { name: "Tonkotsu Ramen", description: "Rich pork broth, chashu, ajitsuke tamago", price: 1480, category: "main",
      ...
      { name: "Chirashi Don", description: "Assorted sashimi over seasoned sushi rice", price: 2280, category: "main",
        image_url: "https://images.unsplash.com/photo-1611143669185-af224c5e3252?w=600" },
      { name: "Matcha Warabimochi", description: "Soft mochi dusted with stone-ground matcha", price: 780, category: "dessert",
      ...
      { name: "Hojicha Latte", description: "Roasted green tea with steamed milk", price: 580, category: "drinks",
        image_url: "https://images.unsplash.com/photo-1545578474-9a93f4d44a92?w=600" },
      { name: "Sapporo Draft", description: "Crisp Japanese lager, 330ml", price: 680, category: "drinks",
